...
/* (gamma_5*gamma_i x gamma_5*gamma_i ) */
void 
mult_a1i_field( int pdir,  int r0[], su3_vector *src, su3_vector *dest ){
  /* operator is gamma_pdir, another (-1)^(x+y+z+t) for antiquark */

  char myname[] = "mult_a1i_field";
  enum gammatype gi5 = dir2g5i(pdir, myname);

  general_spin_taste_op( gi5, gi5, r0, dest, src, NULL);
...
spin_taste_op_links(int index, int r0[], su3_vector *dest, 
  ...
  case a1_x:
    mult_a1i_field(XUP, r0, src, dest);
    break;
  case a1_y:
    mult_a1i_field(YUP, r0, src, dest);
    break;
  case a1_z:
  case a1_i:
    mult_a1i_field(ZUP, r0, src, dest);
    break;
